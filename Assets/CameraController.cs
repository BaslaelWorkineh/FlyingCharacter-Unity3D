...
public class CameraMovement : MonoBehaviour {
	...
	private float rotationSmoothing = 0.9f;
	...
	void LateUpdate () {
		if (target) {
			...
			if (distance < distanceMax) {
				currentDistance = Mathf.Lerp(currentDistance, targetDistance, Time.deltaTime * 10f / Mathf.Abs(targetDistance - currentDistance));
	...
	public void CameraMove () {
		float targetX = x + Input.GetAxis("Mouse X") * xSpeed;
		float targetY = y - Input.GetAxis("Mouse Y") * ySpeed;

		targetX = ClampAngle(targetX, xMinLimit, xMaxLimit);
		targetY = ClampAngle(targetY, yMinLimit, yMaxLimit);

		currentX = Mathf.Lerp(currentX, targetX, rotationSmoothing);
		currentY = Mathf.Lerp(currentY, targetY, rotationSmoothing);
	...
	public void SetTargetDistance(float x) {
		targetDistance  = x;
	}
}